    ...
    *ret_label = ret ? s_label : 0;
    ...
    size_t len =
        ret ? reinterpret_cast<char *>(ret) - reinterpret_cast<char *>(s) + 1
            : n;
    *ret_label =
        taint_union(taint_read_label(s, len), taint_union(s_label, c_label));
  }
  */
  *ret_label = 0;
  return ret;
...
SANITIZER_INTERFACE_ATTRIBUTE char *__dfsw_strrchr(char *s, int c,
  ...
  if (flags().strict_data_dependencies) {
    ...
    *ret_label =
        taint_union(taint_read_label(s, strlen(s) + 1),
                    taint_union(s_label, c_label));
  }
  */
  *ret_label = 0;
...
SANITIZER_INTERFACE_ATTRIBUTE char *__dfsw_strstr(char *haystack, char *needle,
  ...
  if (flags().strict_data_dependencies) {
    ...
    *ret_label =
        taint_union(taint_read_label(haystack, len),
                    taint_union(taint_read_label(needle, strlen(needle) + 1),
                                taint_union(haystack_label, needle_label)));
  }
  */
  *ret_label = 0;
  return ret;
